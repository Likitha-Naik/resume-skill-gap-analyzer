...
import sqlite3, pandas as pd, spacy
...
def register():
    if request.method == 'POST':
        ...
        db = get_db()
        db.execute('INSERT INTO users VALUES (NULL,?,?)',(user,pwd))
        db.commit()
        return redirect('/')
...
def dashboard():
    ...
    if request.method == 'POST':
        ...
        if len(jd_skills) == 0:
            warning = "⚠️ No recognizable skills found in Job Description. Please paste a detailed JD."
        else:
